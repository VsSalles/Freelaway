...
def perfil(request):
    ...
    if usuario.exists():
        messages.add_message(request, messages.ERROR, 'Já existe um usuário com esse Username')
        return redirect('/jobs/perfil')
    usuario = User.objects.filter(email=email).exclude(id=request.user.id)
    if usuario.exists():
        messages.add_message(request, messages.ERROR, 'Já existe um usuário com esse E-mail')
        return redirect('/jobs/perfil')
    ...
    request.user.save()
    messages.add_message(request, messages.SUCCESS, 'Dados alterado com sucesso')
    return redirect('/jobs/perfil')
